...
class Constrained_DQN_LambdaNet_Agent:
    ...
    def __init__(self,
        ...
        self.use_hybrid_net = use_hybrid_net

        # Constrained_DQN_LambdaNet: use_hybrid_net=True
        # Constrained_DQN_PureNet: use_hybrid_net=False
        if self.agent_name in ["Constrained_DQN_PureNet"]:
            print("--- Now {} is running ---".format(self.agent_name))
            self.use_hybrid_net = False
        else:
            print("--- Now {} is running ---".format(self.agent_name))
        ...
        self.lambda_scalar = torch.tensor(1.0, requires_grad=False)

        #tiny neural controller for the λ update
        self.use_lambda_net = use_lambda_net
        self.last_lambda_step = 0.0

        if self.use_lambda_net:
            # feature: [episode_cost, cost_limit, violation]
            self.lambda_net = nn.Sequential(
                nn.Linear(3, lambda_hidden_size),
                nn.ReLU(),
                nn.Linear(lambda_hidden_size, 1),
                nn.Softplus()  # ensures step_size >= 0
            ).to(self.device)
            self.opt_lambda = optim.Adam(
                self.lambda_net.parameters(),
                lr=self.lambda_lr
            )
        else:
    ...
    def optimize_lambda(self, cost_limit, cost):
        ...
        # --- scalar baseline update (no gradients) ---
        violation = float(cost - cost_limit)
        self.lambda_scalar += self.lambda_lr * violation
        self.lambda_lr = self.lambda_lr * 0.99
        self.lambda_scalar = self.lambda_scalar.clamp(min=0.0)

        # --- scalar-only ---
        if self.use_lambda_net is False:
            self.lambda_param = self.lambda_scalar
            return

        # --- lambda-network update ---
        self.cost_avg = cost_limit
        # Build feature vector z_k = [cost, cost_limit, violation]
        z = torch.tensor([
             # float(self.lambda_scalar.item()),
             float(cost),
             float(self.cost_avg),
             float(cost - self.cost_avg),
             ],
            dtype=torch.float32,
            device=self.device
        )

        raw_delta = self.lambda_net(z)


        if self.use_hybrid_net:
            # --- update with scalar and neural net ---
            delta_lambda = 1.0 * torch.tanh(raw_delta)
            lambda_param = (self.lambda_scalar + delta_lambda).clamp(min=0.0)
        else:
            #--- pure neural net update ---
            raw_lambda = raw_delta
            lambda_pre = self.lambda_param.detach()
            lambda_param = raw_lambda.clamp(min=0.0)
            delta_lambda = lambda_param - lambda_pre


        # ===== cost controller sign s =====
        C_scale = max(cost_limit, 1.0)
        v_cost = (cost - cost_limit) / C_scale
        margin_c = 0.0

        s = 0.0
        # too costly, push lambda up
        if v_cost > +margin_c:
            s = +1.0
        # cheap, relax lambda
        elif v_cost < -margin_c:
            s = -1.0
        ...
        # ===== loss =====
        if abs(s) > 0:
            sign_target = torch.tensor(s, dtype=torch.float32, device=self.device)
            L_sign = torch.relu(- sign_target * delta_lambda)
        else:
            L_sign = torch.tensor(0.0, device=self.device)

        L_reg = delta_lambda ** 2

        w_sign = 1.0
        w_reg = 1e-2
        loss_lambda = w_sign * L_sign + w_reg * L_reg

        self.opt_lambda.zero_grad()
        loss_lambda.backward()
        self.opt_lambda.step()

        self.lambda_param = lambda_param.detach()
    ...
    def train(self):
        ...
        while self.steps_done < self.training_steps:
            ...
            if num_episodes % 10 == 0 and self.verbose:
                ...
                print(f"\tgoal = {goal}")
                print(f"lambda_scalar = {self.lambda_scalar.item():.4f}, "
                      f"lambda_param = {self.lambda_param.item():.4f}")
